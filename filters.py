...
all_filters_ = {
    i["word"]: i["answer"] for i in db.get_collection("lordcodes.filters").values()
}
...
async def filter_handler(client: Client, message: Message):
    try:
        ...
        db.set("lordcodes.filters", f"filter_{name}", {"word": word, "answer": text})
...
async def filters_handler(client: Client, message: Message):
    try:
        ...
        for index, a in enumerate(
            db.get_collection("lordcodes.filters").items(), start=1
        ):
            key, item = a
            if key[:6] == "filter":
